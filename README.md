...
By first doing force-body-diagrams on a basic suspension system we obtained equations that described the masses acceleration, using simulink we solved this system to obtain usable metrics on how the tire and car would react to varying road profiles. We then used this data to feed into a simscape physical model to ensure that the output of our model was sensible and get a insight on how they are reacting to different road profiles at different speeds and automatically calculate objective metrics for comfort and road holding.. The road profiles we used to grade out models performance are (Pothole [smooth pothole], Rough road [band-limited noise]), Washboards [wave pattern], Rough Road + speed bump [band-limited noise + smooth speed speedbump) 
